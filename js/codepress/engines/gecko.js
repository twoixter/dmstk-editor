...
CodePress = {
	...
	keyHandler : function(evt) {
		...
		else if(charCode==115 && evt.ctrlKey) { // Ctrl-S
			evt.preventDefault();
			if (window.parent && window.parent.document.forms.length) {
				var _forms = window.parent.document.forms;
				for (i=0; i<_forms.length; i++) {
					var f = _forms[i];
					if (f.className.match('codepress') && f.className.match('autosave')) {
						f.onsubmit();
						f.submit();
						return;
					}
				}
				alert("No se puede auto guardar. Añade class='codepress autosave' al form que contiene el editor.");
			}
		}
